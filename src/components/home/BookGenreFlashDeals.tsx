...
export default function BookGenreFlashDeals({ 
  ...
  const { data: flashProducts = [], isLoading } = useQuery({
    queryKey: ['genre-deals', productType, activeTab],
    queryFn: () => fetchFlashDeals(activeTab, productType),
  ...
  const sourceProducts = (flashProducts as any[]);
  const filteredProducts = productType
    ? sourceProducts.filter((p) => (p as any)?.product_type === productType)
    : excludeTypes.length > 0
      ? sourceProducts.filter((p) => !excludeTypes.includes(((p as any)?.product_type || '').toLowerCase()))
      : sourceProducts.filter((p) => (p as any)?.product_type !== 'books');
    ...
    <div className="w-full bg-white">
      ...
      <div className={`bg-gradient-to-r ${headerGradient} text-white`}>
        <SectionHeader
          title={title}
          icon={icon}
          viewAllLink={viewAllLink || `/search?category=${productType}`}
      ...
      <div className="relative pt-4 px-2">
        {isLoading ? (
          ...
          <div className="text-center py-8 text-gray-500">
            No items available for {genreTabs.find(tab => tab.id === activeTab)?.label}
          </div>
        )}
